...
(function(aufgabe4) {
	...
	function init() {
		fillFieldset();
		document.getElementsByTagName("fieldset")[0].addEventListener("change",
				handleChange);
	...
	function handleChange(_event) {
		...
		childHTML = " ";
		childHTML += (totalPrice = christmasbulbPrice + candlePrice
				+ lamettaPrice + treePrice + holderPrice + shipmentPrice);
		childHTML += " Euro";
		node.innerHTML = childHTML;
	}
	function verify() {
		if (christmasbulbPrice == null || candlePrice == null
				|| lamettaPrice == null || treePrice == null
				|| holderPrice == null || shipmentPrice == null
				|| totalPrice == null || adress.length < 1) {
