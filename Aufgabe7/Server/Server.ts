        ...
        if (_request.url != "/favicon.ico") {
            ...
            for (let i: number = 0; i < urlEncoding.length; i++) {
                if (urlEncoding[i] == "&") { //Alle Artikel die nacheinander mit einen "&" in der Url encodiert wurden werden mit einem Zeilenumbruch belegt.
                    alleArtikel.push(childNodeHTML);
                    childNodeHTML = "<p>";
                }
                else {
                    if (childNodeHTML == "<p>adresse") { //Da die Adressenauswahl kein Artikel darstellt, wird diese in der Else-If Schleife ausgeführt (bedingt durch Zeile 29)
                        childNodeHTML = "<p>Adresse"; //Name vom Text adresse
                    }
                    childNodeHTML += urlEncoding[i];
                }
            }
            alleArtikel.push(childNodeHTML);
